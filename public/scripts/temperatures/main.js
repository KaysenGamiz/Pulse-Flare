...
async function fetchReadingsByWeek(year, month, week) {
  showLoading();
  try {
    const response = await fetch(`/general_utils/temperatures/readings?year=${year}&month=${month}&week=${week}`);
...
async function fetchDevices() {
  try {
    const response = await fetch('/general_utils/temperatures/devices');
    if (!response.ok) throw new Error(`Error HTTP: ${response.status}`);
    const data = await response.json();
    return data.devices || [];
  } catch (error) {
...
async function fetchLiveReading() {
    try {
      const res = await fetch('/general_utils/temperatures/readings/live');
